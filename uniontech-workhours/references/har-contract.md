...
2026-09-23 对当前网页代码再次核对：项目工时的提交会先为任务构造周一至周日完整 7 条数据，再过滤掉已在审批中或已审批的日期；有工时的草稿改为 `status=1`，尚无工时的日期仍以 `hours=null`、`status=0`、`startLog=1`、`saveUpdateFlag=1` 一并发送。只发送有工时的日期可能短暂回读到 `status=1`，随后回退为草稿，因此不能据即时周查询宣称提交成功。提交回读还要核对日历汇总中目标工时已进入 `noApprovedHours`（待审批）或 `havedApprovedHours`（已审批），并间隔后连续确认状态稳定。
