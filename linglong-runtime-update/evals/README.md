...
# 单独测试某个命令
python3 evals/test_dryrun.py version      # 版本推断
...
- **版本推断**: DTK 6.7.45 → 玲珑 6.7.0.45
- **crp-pack**: 验证 `--version` (CRP tag)、`--topic`、`--branch` 等参数传递
...
3. 能正确搜索到 DTK 项目（dtkcore, dtkgui 等）
4. 能列出 crimson-testing 分支的提交信息
5. 创建打包实例成功（HTTP 200）
...
2. 自动 clone 两个仓库到 `~/.cache/linglong-update/repos/`
3. 创建新分支 `update/linglong-runtime-<version>-<timestamp>`
4. webengine 仓库正确应用 `assets/webengine.patch` 补丁
5. `linglong.yaml` 中版本号正确更新（如 6.7.0.45）
...
2. 能正确获取构建编号
3. 能轮询构建状态直到完成
4. 构建成功（result == "SUCCESS"）
5. 从控制台输出中可提取构建产物的 layer URL

**预期结果**: Jenkins 构建成功，产出可访问的 layer 文件。
...
3. 用户确认后，触发 `linglong-runtime-push-to-old` job
4. 用户确认后，触发 `linglong-runtime-push-to-test` job
...
**验证命令**:
```bash
python3 scripts/linglong-update.py push-layer --layer-url http://10.20.64.92:8080/crimson_runtime/stable_test20260722/
```
...
2. 每个步骤完成后状态正确保存到 `~/.config/linglong-update/state.json`
