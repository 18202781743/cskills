...
- **使用场景**: 大多数场景下，调用方只需操作自身用户的配置，使用此接口即可，无需显式指定 uid。例如：应用读取自身 DConfig 配置、系统服务管理当前用户配置等。
...
- **返回值**: `o`（object path）：管理器对象路径
- **使用场景**: 当需要操作非当前连接用户（如其他 uid 用户）的配置时使用。例如：系统管理服务为指定用户创建配置管理器对象、多用户场景下跨用户配置管理等。
